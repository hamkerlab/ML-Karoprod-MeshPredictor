...
class ProjectionPredictor(object):
    """
    ...
    def load_data(self, doe, data, process_parameters, position, output, categorical=[], index='doe_id'):
        ...
        if not len(self.position_attribute) == 2:
    ...
    def _preprocess_variables(self, df):
        ...
        for attr in self.position_attribute:
    ...
    def predict(self, process_parameters, shape):
        ...
        x = np.linspace(self.min_values[self.position_attribute[0]], self.max_values[self.position_attribute[0]], shape[0])
        y = np.linspace(self.min_values[self.position_attribute[1]], self.max_values[self.position_attribute[1]], shape[1])

        positions = np.array([[i, j] for i in x for j in y])

        for i, attr in enumerate(self.position_attribute):
    ...
    def compare(self, doe_id):
        ...
        for idx, attr in enumerate(self.output_attributes):
            plt.figure()
            plt.subplot(121)
            plt.tripcolor(triang, t[:, idx], shading='flat')
            plt.colorbar()
            plt.title("Ground truth - " + attr)
            plt.subplot(122)
            plt.tripcolor(triang, y[:, idx], shading='flat')
            plt.colorbar()
